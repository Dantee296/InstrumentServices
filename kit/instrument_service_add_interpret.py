...
def cmd_channels(rpc):
    ...
    # print("Enter cmd_channels")
    # print("________ cmd_channels identifier: ", rpc._next_identifier)
    ...
    rpc.start()
    # print("________ cmd_channels after rpc.start() rpc._next_identifier: ", rpc._next_identifier)

    # done.wait(5)

    # done.wait 使得工作线程可以执行
    if not done.wait(5):
        print("[WARN] timeout waiting capabilities")

    # print("________ cmd_channels after done.wait(5) rpc._next_identifier: ", rpc._next_identifier)
    rpc.stop()
...
def cmd_sysmontap(rpc):
    ...
    rpc.call("com.apple.instruments.server.services.sysmontap", "setConfig:", {
        ...
        'procAttrs': ['memVirtualSize', 'cpuUsage', 'ctxSwitch', 'intWakeups', 'physFootprint', 'memResidentSize',
                      'memAnon', 'pid', 'powerScore', 'diskBytesRead', 'diskBytesWritten'],
...
def cmd_monitor(rpc, pid, network_stat=True):
    ...
    if network_stat:
        print("start", rpc.call("com.apple.xcode.debug-gauge-data-providers.NetworkStatistics", "startSamplingForPIDs:",
                                {pid}).parsed)
    try:
        while 1:
            if network_stat:
                attr = {}
                ret = rpc.call("com.apple.xcode.debug-gauge-data-providers.NetworkStatistics",
                               "sampleAttributes:forPIDs:", attr, {pid})
...
def cmd_wireless(rpc):
    ...
    print("start",
          rpc.call(channel, "startServerDaemonWithName:type:passphrase:", "perfcat", float(77498864), "U" * 32).parsed)
    enabled = rpc.call(channel, "isServiceEnabled").parsed
    print("enabled", enabled)
    if enabled:
        try:
...
def test(rpc):
    ...
    config = {'OAKeepAllocationStatistics': 'YES', 'DYLD_PRINT_TO_STDERR': '1', 'OAWaitForSetupByPid': "0",
              'DYLD_INSERT_LIBRARIES': '/Developer/Library/PrivateFrameworks/DVTInstrumentsFoundation.framework/liboainject.dylib',
              'OAAllocationStatisticsOutputMask': '0x4f9d0f00', 'OS_ACTIVITY_DT_MODE': '1', 'HIPreventRefEncoding': '1'}

    pid = rpc.call(channel1, "launchSuspendedProcessWithDevicePath:bundleIdentifier:environment:arguments:options:",
                   bundlePath, bundleid, tmp, [], {'StartSuspendedKey': True}).parsed
    ...
    try:
        while 1:
            state += 1
            res = rpc.call(device_channel, "symbolicatorSignatureForPid:trackingSelector:", str(pid),
                           "dyldNotificationReceived:").parsed
...
def start_wireless_mode(device):  # returns (ret, name, preshared_key)
    ...
    try:
        ...
        print("start",
              rpc.call(channel, "startServerDaemonWithName:type:passphrase:", "perfcat_" + name, float(19900724),
                       psk).parsed)
        enabled = rpc.call(channel, "isServiceEnabled").parsed
        # print("enabled", enabled)
...
def get_usb_rpc(device):
    ...
    if not rpc.init(DTXUSBTransport, device):
        return None
    # print("______ get_usb_rpc rpc._next_identifier", rpc._next_identifier)
    # print("_______ get_usb_rpc rpc._cli", rpc._cli)
    return rpc
...
class DTXClientMixin:
    ...
    def _setup_manager(self):
        if hasattr(self, "_dtx_demux_manager"):
            # print("_______ _setup_manager hasattr false")
            return
        # print("_______ _setup_manager hasattr true")
...
class DTXUSBTransport:
    ...
    def send_all(self, client: c_void_p, buffer: bytes) -> bool:
        ...
        self.interpret_dtxMessgae(buffer, "send")
        """
        #将buffer中的数据重定向至send.txt中
        ...
        while buffer:
            """
            instrument_send_command may sent partial data of buffer only, thus, deleting those data from buffer
            is necessary.  
            Obviously, remove those demoded data from buffer by [sent.value: ]
            """
            if instrument_send_command(client, buffer, len(buffer),
                                       pointer(sent)) != InstrumentError.INSTRUMENT_E_SUCCESS:
                return False
    ...
    def recv_all(self, client: c_void_p, length, timeout=-1) -> bytes:
        ...
        """
        create_string_buffer(8192), 8192 is the default size of memory-stack.
        """
        ...
        while len(ret) < length:
            """
            To receive data from client by instrument_receive_with_timeout or instrument_receive.

            The while circle cannot abort until receiving enough data.
            ret is the return value, through rb[:received.value]. 
            """

            err = 0
            left_buffer_length = length - len(ret)
            if left_buffer_length > 8192: left_buffer_length = 8192
            if timeout > 0:
                err = instrument_receive_with_timeout(client, rb, left_buffer_length, pointer(received), timeout)
            else:
                err = instrument_receive(client, rb, left_buffer_length, pointer(received))
        ...
        f.close()
        """
        self.interpret_dtxMessgae(ret, "recv")
        return ret
        pass
...
class InstrumentRPCResult:
    def __init__(self, dtx):
        ...
        # print("___________ InstrumentRPCResult _init_ sel: ", sel)
        if not sel:
            self.xml = ""
            self.plist = ""
            self.parsed = None
        ...
        try:
            ...
            # print("_________ InstrumentRPCResult parese_plist_to_xml\n")
            # print(self.xml)
        except:
            # traceback.print_exc()
        ...
        try:
            ...
            # print("_________ InstrumentRPCResult bplist.parse\n")
            # print(self.plist)
        except:
        ...
        try:
            ...
            # print("_________ InstrumentRPCResult archiver.unarchive\n")
            # print(self.parsed)
        except:
...
class InstrumentRPC:
    ...
    def __init__(self):
        #   _cli instrument服务的socket句柄
        self._cli = None
        #   _is 一个合成类，功能包括DTXMessage的发送和接收，以及instrument client的创建任务
        self._is = None
        #   _recv_thread 负责该服务的线程的标识符,调用threading标准库
        self._recv_thread = None
        #   _running 用于判断该instrument服务是否正在进行
        self._running = False
        #   _callbacks 用于selector部分的回调函数注册
        self._callbacks = {}
        #   _sync_waits 异步事件的存储，类似于异步IO，将事件查询，保存，或者调用
        #   在_call 中通过判断sync来进行是否对事件进行异步操作，在执行注册事件结束之后将其pop删除
        #   在_reciver 接收while循环中，通过判断wait_key(channel_code, identifier)，
        #   决定是否对接收到的信息进行异步操作,
        #   在_reciver 接收while循环结束，通过调用异步_sync_waits每一个元素，进行异步操作的调用
        self._sync_waits = {}
        #   _next_identifier 用于标识rpc服务,与_reciver接收到的dtx中的identifier相关
        self._next_identifier = 1
        #   _channels 用于存储目前RPC instrument服务中的channel标识符，每个channel标识符为唯一，依次增加
        self._channels = {}
        #   _receiver_exiting 用于标识是否停止接收信息
        self._receiver_exiting = False
        #   _unhanled_callback 用于处理未被注册的事件
        self._unhanled_callback = None
        #   _channel_callbacks 用于channel的标识符与其相对应回调绑定的存储
    ...
    def init(self, transport, arg):
        ...
        # DTXClientMixin 负责DTXMessage的发送和接收
        # transport 为DTXUSBTransport，用于创建instrument client服务
        class T(transport, DTXClientMixin):
            pass

        self._is = T()
        # 调用DTXUSBTransport的new_client(), 创建一个新的instrument client
    ...
    def start(self):
        ...
        if self._running:
            return True
        self._running = True
        #   创建专门从rpc server接收信息的线程 InstrumentRecevier
        self._recv_thread = Thread(target=self._receiver, name="InstrumentRecevier")
        self._is.pre_start(self)

        #   开始执行线程, 由于线程受操作系统管理，因此调用recv_thread.start之后并不立即开始执行接收消息线程
        self._recv_thread.start()
        self._is.post_start(self)
        # print("______ start after post_start self._next_identifier: ", self._next_identifier);
        return True
    ...
    def register_callback(self, selector, callback):
        ...
        :return: 无返回值
        """
        # print("_______ register_callback")
    ...
    def register_channel_callback(self, channel, callback):
        ...
        :return: 无返回值
        """
        # print("______ register_channel_callback")
    ...
    def register_unhandled_callback(self, callback):
        ...
        :return: 无返回值
        """
        # print("______ register_unhandled_callback")
    ...
    def call(self, channel: str, selector: str, *auxiliaries):
        # 找出selector str所指的服务频道所对应的channel id，
        # 调用_make_channel是为了解决服务频道存不存在的问题，存在直接返回ID，不存在创建ID
        channel_id = self._make_channel(channel)
        ret = self._call(True, channel_id, selector, *auxiliaries)
        # print("_______ call after _make_channel self._next_identifier: ", self._next_identifier)
    ...
    def _call(self, sync: bool, channel_id: int, selector: str, *auxiliaries):
        ...
        # print("______ _call self._next_identifier: ", self._next_identifier)
        dtx = DTXMessage()
        ...
        for aux in auxiliaries:
            # print("__________ _call aux: ", aux)
        ...
        # print("___________ _call\n")

        """
        dtx_header = dtx.get_dtxMessage_header()
        print("___________ _call")
        print("+++++++++++++++++++  DTXMesageHeader")
        print("DTXMessage magic: ", dtx_header.magic)
        print("DTXMessage cb: ", dtx_header.cb)
        print("DTXMessage fragmentId: ", dtx_header.fragmentId)
        print("DTXMessage fragmentCount: ", dtx_header.fragmentCount)
        print("DTXMessage length: ", dtx_header.length)
        print("DTXMessage identifier: ", dtx_header.identifier)
        print("DTXMessage conversationIndex: ", dtx_header.conversationIndex)
        print("DTXMessage channel_code: ", dtx_header.channelCode)
        print("DTXMessage expect_reply: ", dtx_header.expectsReply)
        print("+++++++++++++++++++  DTXMesageHeader")

        print("\n")

        print("+++++++++++++++++++  DTXMessagePayloadHeader")
        dtx_payloadheader = dtx.get_dtxMessage_payloadHeader()
        if dtx_payloadheader is None:
            print("DTXMessagePayLoad is Node.\n")
        else:
            print("DTXMessagePayloadHeader flags: ", dtx_payloadheader.flags)
            print("DTXMessagePayloadHeader auxiliaryLength: ", dtx_payloadheader.auxiliaryLength)
            print("DTXMessagePayloadHeader totalLength: ", dtx_payloadheader.totalLength)

        print("\n")

        print("DTXMessage selector: ", dtx.get_selector(), dtx.get_selector().hex())
        """
        ...
        if sync:
            ...
            ret = param['result']
            # print("ios => perfcat")
            # print("________  _call in sync hexdump: ")
            (ret.to_bytes())
            self._sync_waits.pop(wait_key)
            return ret

    def _receiver(self):
        # print("_________ _receiver before")
        last_none = 0
        while self._running:
            ...
            if dtx is None:
                ...
                continue
            # print("recv:", dtx)
            # print("______ receiver after self.is.recv_dtx _next_identifier: ", self._next_identifier)
            # print("______ receiver before max() dtx.identifier: ", dtx.identifier)
            self._next_identifier = max(self._next_identifier, dtx.identifier + 1)
            # print("______ receiver after max() _next_identifier: ", self._next_identifier)
            wait_key = (dtx.channel_code, dtx.identifier)
            # print("wait_key: ", wait_key)
            if wait_key in self._sync_waits:
                # print("______ receiver sync_waits:")
                param = self._sync_waits[wait_key]
                # print(param)
                param['result'] = dtx
                param['event'].set()
            elif 2 ** 32 - dtx.channel_code in self._channel_callbacks:
                # print("______ receiver subscription!!")
                # print(dtx.channel_code)
                try:
                    self._channel_callbacks[2 ** 32 - dtx.channel_code](InstrumentRPCResult(dtx))
                except:
                    traceback.print_exc()
            else:
                # print("_____ receiver selector")
                try:
                ...
                if selector and type(selector) is str and selector in self._callbacks:
                    try:
                        ...
                        if dtx.expects_reply:
                            # print("______ receiver expects_reply")
                            reply = dtx.new_reply()
        ...
        # print("___________ After recycle in _receiver.")
        for wait_key in self._sync_waits:
            ...
            print("wait_key result: ", self._sync_waits[wait_key]['result'])
            print("wait_key ")
            print("wait_key event: ", self._sync_waits[wait_key]['event'])
            print("-----------------------------------------------")
...
def main():
    ...
    # print("-----device")
    # print(opts) illustrates the device id and which kind of cmd will be operated
    print(opts)
